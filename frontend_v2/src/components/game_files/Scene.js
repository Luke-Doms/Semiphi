...
export class Scene {
  ...
  async Load() {
    ...
    if (puzzleParams?.camera) {
      ...
      this.eye = new Camera([4.5 * this.x, 4.5 * this.y, 4.5 * this.z], [0, 0, 1]);
    }
  ...
  Unload() {
    ...
    this.gl.canvas.removeEventListener("pointerup", this.OnPointerUp);
  ...
  Begin() {
    ...
    this.gl.canvas.addEventListener("pointerup", this.OnPointerUp);
